...
public class Controller {
    ...
    public ResponseEntity<Conta> atualizarConta(@PathVariable(value = "id") int id, @RequestBody Conta updateConta) {
        try {
            // UPDATE Conta SET id=?2, agencia=?3, numero_conta=?4, codigo_banco=?5 WHERE id
            // = ?1
            this.contaRepository.queryUpdate(id, updateConta.getId(), updateConta.getAgencia(),
                    updateConta.getNumero_conta(), updateConta.getCodigo_banco());
        } catch (Exception e) {
            System.err.println(e);
            return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);// ERRO
        }
